...
function Checkout() {
  ...
  const [layout, setLayout] = useState(<div className="wrapper"></div>);
  ...
  if (!UserService.isAuthenticated())
    return (
      <div className="wrapper">
        <Text
          variant="h2"
          children={"debes iniciar sesion para poder comprar."}
        />
      </div>
    );
  ...
  useEffect(() => {
    ...
    ProductosService.getProductoById(intID)
      .then((data) => {
        TipoPagoService.getAllTipoPago()
          .then((tiposPago) => {
            let options = [];
                    ...
                    <Col>
                      ...
                      <div style={{ display: "flex", gap: ".5rem" }}>
                        ...
                        <Button
                          ...
                          children={`Finalizar Compra? (stock:${data.stock})`}
                          onClick={() => {
                            setLayout(
                              <div className="wrapper">
                                <Text
                                  variant="h2"
                                  children={"Cargando Compra.."}
                                />
                              </div>
                            );
                            ...
                            ProductosService.updateProducto(data.id, data).then(
                              () => {
                                ...
                                const now = new Date();
                                const fechaPedido = now.toISOString().slice(0, 19);
                                const totalPedido = data.precio;
                        ...
                        <select
                          name="Tipo Pago"
                          id="checkout"
                          defaultValue={pagoType}
                          onChange={(ev) => {
                            setPagoType(parseInt(ev.target.value));
                          }}
                        >
                          {options}
                        </select>
